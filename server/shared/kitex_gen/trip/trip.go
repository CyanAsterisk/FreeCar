...
func (p *GetTripsRequest) ReadField1(iprot thrift.TProtocol) error {
	...
	var _field base.TripStatus
	if v, err := iprot.ReadI32(); err != nil {
		return err
	} else {
		_field = base.TripStatus(v)
	}
	p.Status = _field
	return nil
}
...
func (p *GetTripsResponse) ReadField2(iprot thrift.TProtocol) error {
	...
	_field := make([]*base.TripEntity, 0, size)
	values := make([]base.TripEntity, size)
	for i := 0; i < size; i++ {
		_elem := &values[i]
		_elem.InitDefault()
...
func (p *TripServiceGetSomeTripsResult) ReadField0(iprot thrift.TProtocol) error {
	_field := NewGetSomeTripsResponse()
	if err := _field.Read(iprot); err != nil {
		return err
	}
	p.Success = _field
